package mak.capture.mssql;

public class MsTypes {
	public static final int IMAGE = 34;
	public static final int TEXT = 35;
	public static final int UNIQUEIDENTIFIER = 36;
	public static final int DATE = 40;
	public static final int TIME = 41;
	public static final int DATETIME2 = 42;
	public static final int DATETIMEOFFSET = 43;
	public static final int TINYINT = 48;
	public static final int SMALLINT = 52;
	public static final int INT = 56;
	public static final int SMALLDATETIME = 58;
	public static final int REAL = 59;
	public static final int MONEY = 60;
	public static final int DATETIME = 61;
	public static final int FLOAT = 62;
	public static final int SQL_VARIANT = 98;
	public static final int NTEXT = 99;
	public static final int BIT = 104;
	public static final int DECIMAL = 106;
	public static final int NUMERIC = 108;
	public static final int SMALLMONEY = 122;
	public static final int BIGINT = 127;
	public static final int HIERARCHYID = 128;
	public static final int GEOMETRY = 129;
	public static final int GEOGRAPHY = 130;
	public static final int VARBINARY = 165;
	public static final int VARCHAR = 167;
	public static final int BINARY = 173;
	public static final int CHAR = 175;
	public static final int TIMESTAMP = 189;
	public static final int NVARCHAR = 231;
	public static final int NCHAR = 239;
	public static final int XML = 241;
	public static final int SYSNAME = 256;
}
