...
type
  TIntToHexS = function(address: UINT_PTR; chars: integer; signed: boolean = false; signedsize: integer = 0): string of object;
...
//    datatype:
  ...
  TDisassembleEvent = function(sender: TObject; address: UINT_PTR; var ldd: TLastDisassembleData; var output: string; var description: string): boolean of object;
  ...
  private
    ...
    function inttohexs_withoutsymbols(value: UINT_PTR; chars: integer; signed: boolean = false; signedsize: integer = 0): string;
  ...
  public
    ...
    function disassemble(var offset: UINT_PTR; var description: string): string;
    function getLastBytestring: string;
    constructor Create(isX64: Boolean = False);
...
function isAddress(address: UInt_Ptr): boolean;

var
  processhandle: THandle = Thandle(-1);
...
begin
  case getreg(bt) of
    ...
    7:
      result := 'dr7';
    8:
      result := 'dr8'; //Do not excist, but let's implement the encoding
    9:
      result := 'dr9';
    10:
      result := 'dr10';
...
function TDisassembler.Rex_B: boolean;
begin
  result := (RexPrefix and BIT_REX_B) = BIT_REX_B;
end;

function TDisassembler.Rex_X: boolean;
begin
  result := (RexPrefix and BIT_REX_X) = BIT_REX_X;
end;

function TDisassembler.Rex_R: boolean;
begin
  result := (RexPrefix and BIT_REX_R) = BIT_REX_R;
end;

function TDisassembler.Rex_W: boolean;
begin
...
begin
      ...
      1:
        begin
          ...
          case getrm(memory[modrmbyte]) of
            0:
              begin
                if shortint(memory[modrmbyte + 1]) >= 0 then
                  result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'ax' + endcolor + '+' + inttohexs(memory[modrmbyte + 1], 2) + '],'
                else
            ...
            1:
              if shortint(memory[modrmbyte + 1]) >= 0 then
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'cx' + endcolor + '+' + inttohexs(memory[modrmbyte + 1], 2) + '],'
              else
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'cx' + endcolor + inttohexs(shortint(memory[modrmbyte + 1]), 2) + '],';

            2:
              if shortint(memory[modrmbyte + 1]) >= 0 then
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'dx' + endcolor + '+' + inttohexs(memory[modrmbyte + 1], 2) + '],'
              else
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'dx' + endcolor + inttohexs(shortint(memory[modrmbyte + 1]), 2) + '],';

            3:
              if shortint(memory[modrmbyte + 1]) >= 0 then
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'bx' + endcolor + '+' + inttohexs(memory[modrmbyte + 1], 2) + '],'
              else
          ...
          case getrm(memory[modrmbyte]) of
            0:
              if integer(dwordptr^) >= 0 then
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'ax' + endcolor + '+' + inttohexs(dwordptr^, 8) + '],'
              else
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'ax' + endcolor + '-' + inttohexs(-integer(dwordptr^), 8) + '],';

            1:
              if integer(dwordptr^) >= 0 then
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'cx' + endcolor + '+' + inttohexs(dwordptr^, 8) + '],'
              else
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'cx' + endcolor + '-' + inttohexs(-integer(dwordptr^), 8) + '],';

            2:
              if integer(dwordptr^) >= 0 then
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'dx' + endcolor + '+' + inttohexs(dwordptr^, 8) + '],'
              else
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'dx' + endcolor + '-' + inttohexs(-integer(dwordptr^), 8) + '],';

            3:
              if integer(dwordptr^) >= 0 then
                result := getsegmentoverride(prefix) + '[' + colorreg + regprefix + 'bx' + endcolor + '+' + inttohexs(dwordptr^, 8) + '],'
              else
        ...
        begin
          case getrm(memory[modrmbyte]) of
            0:
              case inst of
                0:
                  if rex_w or (opperandsize = 64) then
                    result := 'rax'
                  else
                    result := 'eax';
                1:
  ...
  if last <> (Dword(modrmbyte) + 1) then //add an extra seperator since some bytes have been added, usually the last one, except when the opcode has a immeadiate value followed, which this seperator will then separate
  begin
    LastDisassembleData.Seperators[LastDisassembleData.SeperatorCount] := last;
    inc(LastDisassembleData.SeperatorCount);
  end;
  ...
  if last > 15 then
    messagebox(0, pchar(result), nil, 0);
end;
...
begin
  case inst of
    0:
      LastDisassembleData.datasize := pointersize;
    1:
...
function TDisassembler.disassemble(var offset: UINT_PTR; var description: string): string;
var
  memory: TMemory;
  actualread: UINT_PTR;
  startoffset: UINT_PTR;
  tempresult: string;
  wordptr: ^word;
  dwordptr: ^dword;
  i, j: integer;
...
//the following 2 were made up by me.
            ...
            $02:
              begin
                ...
                if $66 in prefix2 then
                  lastdisassembledata.parameters := r16(memory[2]) + ',' + modrm(memory, prefix2, 2, 1, last)
                else
            ...
            $b6:
              begin
                ...
                if $66 in prefix2 then
                  lastdisassembledata.parameters := r16(memory[2]) + ',' + modrm(memory, prefix2, 2, 2, last, 8)
                else
            ...
            $b7:
              begin
                ...
                lastdisassembledata.opcode := 'movzx';
                if $66 in prefix2 then
                  lastdisassembledata.parameters := r16(memory[2]) + ',' + modrm(memory, prefix2, 2, 1, last, 16)
                else
...
//---------
      ...
      $40..$47:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.parameters := rd16(memory[0] - $40)
          else
      ...
      $48..$4f:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.parameters := rd16(memory[0] - $48)
          else
      ...
      $50..$57:
        begin
          description := 'push word or doubleword onto the stack';
          if is64bit then
            rexprefix := rexprefix or bit_rex_w; //so rd will pick the 64-bit version

          lastdisassembledata.opcode := 'push';
          if $66 in prefix2 then
            lastdisassembledata.parameters := rd16(memory[0] - $50)
          else
      ...
      $58..$5f:
        begin
          ...
          lastdisassembledata.opcode := 'pop';
          if $66 in prefix2 then
            lastdisassembledata.parameters := rd16(memory[0] - $58)
          else
      ...
      $60:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'pusha'
          else
      ...
      $61:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'popa'
          else
      ...
      $6d:
        begin
          ...
          description := 'input from port to string';
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'insw'
          else
      ...
      $6f:
        begin
          ...
          description := 'output string to port';
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'outsw'
          else
...
//                      offset:=offset+last;
      ...
      $9c:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'pushf'
          else
          begin
            if is64bit then
      ...
      $9d:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'popf'
          else
          begin
            if is64bit then
      ...
      $ab:
        begin
          description := 'store string';
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'stosw'
          else
          begin
            if rex_w then
      ...
      $ad:
        begin
          description := 'load string';
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'lodsw'
          else
          begin
            if rex_w then
      ...
      $af:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'scasw'
          else
          begin
            if rex_w then
...
{^^^^^^^^^^^^^^^^^^}
      ...
      $cf:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.opcode := 'iret'
          else
          begin
            if rex_w then
      ...
      $e5:
        begin
          ...
          if $66 in prefix2 then
            lastdisassembledata.parameters := colorreg + 'ax' + endcolor + ',' + inttohexs(memory[1], 2)
          else
            lastdisassembledata.parameters := colorreg + 'eax' + endcolor + ',' + inttohexs(memory[1], 2);
          inc(offset);
      ...
      $ed:
        begin
          ...
          lastdisassembledata.opcode := 'in';
          if $66 in prefix2 then
            lastdisassembledata.parameters := colorreg + 'ax' + endcolor + ',' + colorreg + 'dx' + endcolor
          else
      ...
      $ef:
        begin
          ...
          lastdisassembledata.opcode := 'out';
          if $66 in prefix2 then
            lastdisassembledata.parameters := colorreg + 'dx' + endcolor + ',' + colorreg + 'ax' + endcolor
          else
...
begin
  ...
  begin
    case opcode[3] of
      't':
        result := opcode4FST(opcode); //fstxxx  (fst, fstp, fstcw, fstenv, fstsw)
    end;
  end;
end;
  ...
  begin
    case opcode[2] of
      'i':
        result := 2; //fixxxxx
      'n':
        result := opcode3FN(opcode);
      's':
        result := opcode3FS(opcode);
    end;
  end;
end;
...
constructor TDisassembler.Create(isX64: Boolean);
begin
  is64bit := isX64;
  if isX64 then
  begin
...
function TDisAssembler.inttohexs_withoutsymbols(value: UINT_PTR; chars: integer; signed: boolean = false; signedsize: integer = 0): string;
begin
  if chars = 2 then
  begin
    signed := true;
