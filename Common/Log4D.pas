...
{ Levels ----------------------------------------------------------------------}
  ...
  public
    ...
    class function GetLevel(Name: string; DefaultLevel: TLogLevel): TLogLevel; overload;
  end;
...
{ Events ----------------------------------------------------------------------}
  ...
  public
    constructor Create(const Logger: TLogLogger; const Level: TLogLevel; const Message: string; const Err: Exception; const TimeStamp: TDateTime = 0); overload;
    constructor Create(const Logger: TLogLogger; const Level: TLogLevel; const Message: TObject; const Err: Exception; const TimeStamp: TDateTime = 0); overload;
...
{ ErrorHandler ----------------------------------------------------------------}
  ...
  public
    ...
    procedure Error(const Message: string; const Err: Exception; const ErrorCode: Integer; const Event: TLogEvent = nil); overload; virtual; abstract;
  end;
  ...
  public
    ...
    procedure Error(const Message: string; const Err: Exception; const ErrorCode: Integer; const Event: TLogEvent = nil); overload; override;
  end;
...
function PropertiesStrToBool(Value: string; const Default: Boolean): Boolean;
...
{ Retrieve a level object given its name, or default if not valid. }
class function TLogLevel.GetLevel(Name: string; DefaultLevel: TLogLevel): TLogLevel;
var
  Index: Integer;
begin
  Result := DefaultLevel;
...
constructor TLogEvent.Create(const Logger: TLogLogger; const Level: TLogLevel; const Message: string; const Err: Exception; const TimeStamp: TDateTime);
begin
  inherited Create;
  FLogger := Logger;
  FLevel := Level;
...
constructor TLogEvent.Create(const Logger: TLogLogger; const Level: TLogLevel; const Message: TObject; const Err: Exception; const TimeStamp: TDateTime);
var
...
begin
  ...
  if ErrorMessage <> '' then
    Result := Result + '<tr><td bgcolor="#993300" ' + 'style="color: White; font-size: xx-small;" colspan="6">' + ErrorMessage + '</td></tr>' + CRLF;
end;
...
type
  TPatternPart = (ppText, ppLogger, ppClassName, ppDate, ppException, ppFileName, ppLocation, ppLine, ppMessage, ppMethod, ppNewLine, ppLevel, ppRuntime, ppThread, ppNDC, ppPercent);
...
procedure TLogOnlyOnceErrorHandler.Error(const Message: string; const Err: Exception; const ErrorCode: Integer; const Event: TLogEvent);
begin
  if not FSeenError then
...
begin
  ...
  for Index := 0 to FLoggers.Count - 1 do
    ...
    begin
      LogLog.Debug(Format(FallbackReplaceMsg, [FAppender.Name, FBackupAppender.Name, Name]));
      RemoveAppender(FAppender);
      AddAppender(FBackupAppender);
    end;
end;

procedure TLogFallbackErrorHandler.Error(const Message: string; const Err: Exception; const ErrorCode: Integer; const Event: TLogEvent);
begin
  Error(Format('%s - (%d) %s', [Message, Err.Message, ErrorCode]));
end;
...
begin
  ...
  if Name = AcceptMatchOpt then
    FAcceptOnMatch := PropertiesStrToBool(Value, True);
end;
...
begin
  if (MaxLevel <> nil) and (MaxLevel.Level >= Event.Level.Level) and (MinLevel <> nil) and (MinLevel.Level <= Event.Level.Level) then
    Result := Acceptance[AcceptOnMatch]
  else
    Result := fdNeutral;
end;
...
begin
  ...
  if Name = MatchOpt then
    ...
    FIgnoreCase := PropertiesStrToBool(Value, FIgnoreCase);
end;
...
begin
  ...
  try
    ...
    begin
      FAppend := PropertiesStrToBool(Value, FAppend);
    end
...
procedure TLogRDFAppender.CloseLogFile;
begin
 if FStream <> nil then
    FreeAndNil(FStream);
end;
...
begin
  ...
  begin
    // Check if directory exists
    fLpath := ExtractFilePath(FFileName);
    if (fLpath <> '') and not DirectoryExists(fLpath) then
      ForceDirectories(fLpath);
    // Creating new file
    AssignFile(f, FFileName);
    ReWrite(f);
    CloseFile(f);
...
procedure TLogRDFAppender.SetOption(const Name, Value: string);
...
begin
  if Debug <> '' then
    LogLog.InternalDebugging := PropertiesStrToBool(Debug, False);
...
procedure RegisterClass(ClassType: TClass; InterfaceType: TGUID; InterfaceName: string; Names: TStringList; Classes: TClassList);
var
  Index: Integer;
begin
...
function FindClass(ClassName: string; InterfaceType: TGUID; Names: TStringList; Classes: TClassList): IUnknown;
var
  Index: Integer;
...
{$ENDIF}
  ...
  RegisterAppender(TLogRDFAppender);
